...
import hapi.application.C_GenerationHTML;
 ...
 * @author Cédric
 */
public class BD_GenererHTML extends JDialog implements FenetreHAPI
{
	private static final long serialVersionUID = 2163633555302594769L;
    //Panels
	...
	//Controleur
	private C_GenerationHTML cGeneration = null;
	...
	public BD_GenererHTML(JFrame parent, String lIdProcessus)
	{
		//Association au parent
		super(parent, Bundle.getText("BD_GenererHTML_caption"), true);
	...
	public void operationSurBoutons()
	{
		...
		//Creer
		BP_Creer.setMnemonic(Bundle.getChar("BD_GenererHTML_creer_mne"));
		BP_Creer.setText(Bundle.getText("BD_GenererHTML_creer"));
		BP_Creer.addActionListener(actionCreer);
		//Ecraser
		BP_Ecraser.setMnemonic(Bundle.getChar("BD_GenererHTML_ecraser_mne"));
		BP_Ecraser.setText(Bundle.getText("BD_GenererHTML_ecraser"));
	...
	{
		//Liste des fichiers
		STC_Fichiers.setText(Bundle.getText("BD_GenererHTML_liste"));
		//Parcourir
		...
		//Chemin
		STC_Chemin.setText(Bundle.getText("BD_GenererHTML_chemin"));
		LD_Chemin.setPreferredSize(new Dimension(350, 24));
		LD_Chemin.setEditable(true);
		LD_Chemin.addActionListener(actionRechercher);
		//Rechercher
		BP_Rechercher.setText(Bundle.getText("BD_GenererHTML_rechercher"));
	...
	{
		BD_ImporterCSS importCSS = new BD_ImporterCSS(this);
		importCSS.setVisible(true);
		cGeneration = new C_GenerationHTML(idProcessus,importCSS.getNomFichierTemp());
		LS_Fichiers.setModel(cGeneration.getModeleFichiers());
		LD_Chemin.setModel(cGeneration.getModeleChemin());
	...
	private void doExporter(final String nomDuFichier)
	{
		...
		cGeneration.addChemin(lAdresse);
		cGeneration.sauvegarderSelectionChemin(lAdresse);

		//Création de la fenêtre de téléchargement
		final BM_Telechargement bmTelechargement = new BM_Telechargement(this, Bundle.getText("BD_GenererHTML_encours"), cGeneration);

		try
		{

			final BD_LoginPwd leLogin = new BD_LoginPwd(this, lAdresse, "HTM");
			...
			Thread leTraitement = new Thread()
			{
				public void run()
				{
					...
					try
					{
						...
						{
							cGeneration.uploaderFichier(nomDuFichier, lAdresse);
						}
						else
						{
							try
							{
								cGeneration.uploaderFichier(nomDuFichier, lAdresse, C_Utilisateur.findServeurHTML(lAdresse).getLogin(), C_Utilisateur.findServeurHTML(lAdresse).getMotDePasse());
							}
							catch (LoginFTPException e)
							{
								leLogin.setVisible(true);
								cGeneration.uploaderFichier(nomDuFichier.toString(), (String) LD_Chemin.getSelectedItem(), C_Utilisateur.findServeurHTML(lAdresse).getLogin(), C_Utilisateur.findServeurHTML(lAdresse).getMotDePasse());
							}
						}
						JOptionPane.showMessageDialog(bmTelechargement, Bundle.getText("BD_GenererHTML_Termine"), Bundle.getText("Information"), JOptionPane.INFORMATION_MESSAGE);
					}
			...
			//Nommage du thread
			leTraitement.setName("Génération page web");
			//Démarrage
			leTraitement.start();
			//Affichage
	...
	private void doRechercher()
	{
		...
		if (lAdresse != null)
			if (!lAdresse.equals(""))
			{
				...
				cGeneration.effacerListeFichiers();
				try
				{
					if (leChemin.isDirectory())
					{
						//récupération de la liste des fichiers
						cGeneration.getListeFichiersLocal("html", lAdresse, "", "");
						cGeneration.getListeFichiersLocal("HTML", lAdresse, "", "");
					}
					else
					{
						...
						try
						{
							cGeneration.getListeFichiers("html", lAdresse, cGeneration.getLogin(lAdresse), cGeneration.getPWD(lAdresse));
							cGeneration.getListeFichiers("HTML", lAdresse, cGeneration.getLogin(lAdresse), cGeneration.getPWD(lAdresse));
						}
						catch (LoginFTPException e)
						{
							new BD_LoginPwd(this, lAdresse, "HTM").setVisible(true);
							cGeneration.getListeFichiers("html", lAdresse, cGeneration.getLogin(lAdresse), cGeneration.getPWD(lAdresse));
							cGeneration.getListeFichiers("HTML", lAdresse, cGeneration.getLogin(lAdresse), cGeneration.getPWD(lAdresse));
						}
	...
	private void doParcourir()
	{
		...
		fileChooser.setDialogTitle(Bundle.getText("BD_GenererHTML_parcourir"));
		fileChooser.setFileSelectionMode(JFileChooser.FILES_AND_DIRECTORIES);
		fileChooser.setAcceptAllFileFilterUsed(false);
		fileChooser.setFileFilter(new FiltreFichier("html", Bundle.getText("HTML")));
		...
		if (res == JFileChooser.APPROVE_OPTION)
		{
			...
			{
				cGeneration.addChemin(fileChooser.getSelectedFile().getAbsolutePath().substring(0, fileChooser.getSelectedFile().getAbsolutePath().lastIndexOf(File.separator)));
			...
			{
				cGeneration.addChemin(fileChooser.getSelectedFile().getAbsolutePath());
	...
	{
		String nomDuFichier = JOptionPane.showInputDialog(this, Bundle.getText("BD_GenererHTML_input"));
		if (!((nomDuFichier == null) || (nomDuFichier.equals(""))))
		{
			nomDuFichier = nomDuFichier + ".html";
			File unFichier = new File((String) LD_Chemin.getSelectedItem() + File.separator + nomDuFichier);
			if (unFichier.isFile())
			{
				if (JOptionPane.showConfirmDialog(this, Bundle.getText("BD_GenererHTML_existe"), Bundle.getText("Question"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION)
